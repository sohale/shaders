...
vec2 np_cloud(float t, int num) {
    ...
    float ym = 0.8+ sin(0.5+t*0.01)*0.02;
    np = vec2(cc.x0-fract(cc.px) *1.5 , ym); // x : -1 1
    }
    ...
    cc.x0 = 1.4;
    cc.px = (t + 0.75)*0.0045;
    float ym = 0.88+ sin(0.75+t*0.01)*0.03;
     np = vec2(cc.x0-fract(cc.px) *1.5 , ym); // x : -1 1
    }
    ...
    float ym = 0.85+ sin(0.5+t*0.042)*0.0095;
   	np = vec2(cc.x0-fract(cc.px) *1.5 , ym); // x : -1 1
    }
